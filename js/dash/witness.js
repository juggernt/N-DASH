...
function showWitnessAddress(wuuid) {
	...
	var qrcode = new QRCode("witnessaddressqr", {
	    text: "holder:"+nlgwaddresslist[wuuid]['address']+"?label="+nlgwaddresslist[wuuid]['label'],
	    width: 128,
	    height: 128,
	    colorDark : "#000000",
	    colorLight : "#ffffff",
	...
	$('#witnessaddresslinkqr').html("<a href='holder:"+nlgwaddresslist[wuuid]['address']+"?label="+nlgwaddresslist[wuuid]['label']+"' target='_blank'>holder:"+
	...
	$.post( "ajax/witnessactions.php?action=createaccount", { accountname: newaccountnametosend, addpassword: addpasswordtosend })
	 .done(function( data ) {
	 	...
	 	if(data['message']==undefined) {
	 		$('#creationmessage').html("<div class='alert alert-info'>Holding account created</div>");
...
//	if($('#lockamount').val == "" || ($.isNumeric($('#lockamount').val()) && $.isNumeric(slidervalue))) {
...
function withdrawWitnessFunds() {
	...
	$.post( "ajax/witnessactions.php?action=withdrawif", { fromlabel: wifFromAccount, tolabel: wifToAccount, pass: wifWalletPassword })
		 .done(function( data ) {
		 	...
		 	if(data=="1") {
		 		$('#wiftransactionmessage').html("<div class='alert alert-success'>Withdrawing initial funding from holding account. It can take a while before this is visible.</div>");
...
// @1.0		$.post( "ajax/witnessactions.php?action=fundaccount", { tolabel: confirmwitnessaccount, amount: transactionamount, pass: transactionpassphrase, fromlabel: confirmguldenaccount, locktime: confirmtime })
		$.post( "ajax/witnessactions.php?action=fundaccount", { tolabel: confirmwitnessaccount, amount: transactionamount, pass: transactionpassphrase, fromlabel: confirmguldenaccount, locktime: confirmtime, extend: extendFund }) // @1.0
		 .done(function( data ) {
		 	var data = jQuery.parseJSON(data);
		 	
		 	if(data=="1") {
		 		$('#conftransmes').html("<div class='alert alert-success'>Funding holder account. It can take a while before this is visible.</div>");
				...
				setTimeout(function(){ 
		 		...
		 		$('#conftransmes').html("<div class='alert alert-warning'>Invalid Novo address.</div>");
		 		$('#confirmtransactionpass').val("");
		 		$('#confirmedcancel').prop("disabled", false);
		 	} else if(data=="-6") {
		 		...
		 		$('#conftransmes').html("<div class='alert alert-warning'>Cannot fund a holding-only (imported) holder account.</div>");
		 		$('#confirmtransactionpass').val("");
		 		$('#confirmedcancel').prop("disabled", false);
		 	} else if(data=="-8") {
...
function withdrawWitnessEarnings() {
	...
	$.post( "ajax/witnessactions.php?action=withdrawearnings", { toaddress: earningstoaddress, pass: earningswithdrawpass, fromlabel: earningsfromaccount })
	 .done(function( data ) {
	 	...
	 	if(data=="1") {
	 		$('#withdrawmessage').html("<div class='alert alert-success'>Withdrawing holding earnings.</div>");
			...
			setTimeout(function(){ 
	 		...
	 		$('#withdrawmessage').html("<div class='alert alert-warning'>Invalid Novo address.</div>");
	 	} else if(data=="-6") {
	 		$('#withdrawmessage').html("<div class='alert alert-warning'>Insufficient funds.</div>");
	 	} else if(data=="-7") {
	 		$('#withdrawmessage').html("<div class='alert alert-warning'>Cannot fund a holding-only (imported) holding account.</div>");
	 	} else if(data=="-8") {
...
// @1.0	var lockedamount = parseFloat($('#lockamount').val());
// @1.0->
    var locked = $('#lockamount').val();
    if (locked == "") locked = 0;
	var lockedamount = parseFloat(locked);
	...
	$.post( "ajax/witnessactions.php?action=importaccount", { importaccountname: importaccountnametosend, importaccountkey: importaccountkeytosend, importpass: importpass })
	 .done(function( data ) {
	 	...
	 	if(data['message']==undefined || data['message']==null) {
	 		$('#keyimportmessage').html("<div class='alert alert-info'>Holding account imported. Novo will now rescan. See the overview page for the progress.</div>");
	 		setTimeout(function(){ 
	  ...
	  $.getJSON( "ajax/witness.php?account="+waccountuuid, function( data ) {				 
		  ...
		  if(data['server']['cpu']!='') {
		  	  if(data['disablewallet']=="1") {
		  	  	$('#witnessnetworkpanel').html("Wallet disabled in Novo configuration");
		  	  } else {
		  	  	
		  	  	totalNetworkWeight = data['witness']['totalNetworkWeight'];
		  	  	
// @1.0		  	  	$('#totalwitnesses').html(data['witness']['totalwitnesses']);
		  	  	$('#totalwitnesses').html(data['witness']['totalholders']); // @1.0
// @1.0		  	  	$('#totalwitnessweight').html((totalNetworkWeight/1000000).toFixed(2)+" Mil");
		  	  	$('#totalwitnessweight').html(totalNetworkWeight); // @1.0
		  	  	$('#pow2phase').html("Phase "+data['witness']['currentPhase']);
// @1.0		  	  	$('#totalwitnesslocked').html((data['witness']['totalGuldenLocked']/1000000).toFixed(2)+" Mil");
		  	  	$('#totalwitnesslocked').html(data['witness']['totalGuldenLocked'].toFixed(2)); // @1.0
		  	  	...
		  	  	if(data['witness']['currentPhase'] > 1) {
		  	  		$('#witnessstatuspanel').html("<ul>" +
		  	  									  ...
		  	  									  "<li><a data-toggle='modal' href='#importwitnessaccount'>Import holding key</a></li>" +
...
// Comment for testing extend witness account->		  
                    if (data['witnessaccountdetails'][value['label']]['status'] != 'Not funded') {
                        lockAmount = data['witnessaccountdetails'][value['label']]['amount'];
                        monthsLeft = data['witnessaccountdetails'][value['label']]['estimated_witness_period']/(576*30)>>0;
                        $("#slider").slider("option", "min", monthsLeft+1);
                        $("#locktime").text(monthsLeft+1);
                    }
                    else {
                        $("#slider").slider("option", "min", 1);
                        $("#locktime").text(slidervalue);
                    }
// Comment for testing<-
/* Uncomment for testing->
                    var test = 1;
                    if (lockAmount == 0) lockAmount = 100;
                    if (monthsLeft == 0) { // Remove
                        monthsLeft = 100519/(576*30)>>0;
                        $("#slider").slider("option", "min", monthsLeft+1);
                        $("#locktime").text(monthsLeft+1);
                    }
                    totalNetworkWeight = 58000;
// Uncomment for testing!! */          
			        	...
			        	witnesspanelbody += "<ul class='dropdown-menu' aria-labelledby='" + moreinfocollapsename + "actions'>";
			        		//Witness actions
				  	  		...
				  	  		if(data['witness']['currentPhase'] > 1) {
				  	  			...
				  	  			if(data['witnessaccountdetails'][value['label']]['status'] == "Not funded") {
// Testing				  	  			if(data['witnessaccountdetails'][value['label']]['status'] == "Not funded" && !test) {
					  	  			witnesspanelbody += "<li><a data-toggle='modal' href='#fundwitnessaccount' onclick=\"changeWitnessAccount('"+value['UUID']+"')\"><i class='glyphicon glyphicon-import'></i> Fund holder account</a></li>";
// @1.0->
                                    extendFund = 0;
                                    $('#fundingtitle').html("Fund holding account");
                                    if (lockAmount != 0) $("#lockamount").val(lockAmount);
// @1.0<-
					  	  			witnesspanelbody += "<li><a data-toggle='modal' href='#deleteaccount' onclick=\"changeWitnessAccount('"+value['UUID']+"')\"><i class='glyphicon glyphicon-trash'></i> Delete holder account</a></li>";
					  	  		} else {
					  	  			...
					  	  			if(data['witnessaccountdetails'][value['label']]['witnessonly'] == false) {
						  	  			witnesspanelbody += "<li><a data-toggle='modal' href='#exportwitnesskey' onclick=\"changeWitnessAccount('"+value['UUID']+"')\"><i class='glyphicon glyphicon-share'></i> Get holder keys</a></li>";
					  	  			...
					  	  			witnesspanelbody += "<li><a data-toggle='modal' href='#renameaccount' onclick=\"changeWitnessAccount('"+value['UUID']+"')\"><i class='glyphicon glyphicon-pencil'></i> Rename holder account</a></li>";
					  	  			witnesspanelbody += "<li><a data-toggle='modal' href='#witnessearningsgraph' onclick=\"createProjectionGraph('"+value['UUID']+"')\"><i class='glyphicon glyphicon-object-align-bottom'></i> Show holder earnings graph</a></li>";
					  	  			witnesspanelbody += "<li><a data-toggle='modal' href='#witnessaddress' onclick=\"showWitnessAddress('"+value['UUID']+"')\"><i class='glyphicon glyphicon-qrcode'></i> Show holder address</a></li>";
					  	  			witnesspanelbody += "<li class='divider'></li>";
					  	  			witnesspanelbody += "<li><a data-toggle='modal' href='#deleteaccount' onclick=\"changeWitnessAccount('"+value['UUID']+"')\"><i class='glyphicon glyphicon-trash'></i> Delete holder account</a></li>";
// @1.0->
                                    witnesspanelbody += "<li><a data-toggle='modal' href='#fundwitnessaccount' onclick=\"changeWitnessAccount('"+value['UUID']+"')\"><i class='glyphicon glyphicon-resize-full'></i> Extend holder account</a></li>";
                                    extendFund = 1;
                                    $('#fundingtitle').html("Extend funding account");
                                    $("#lockamount").val(lockAmount);
                                    $('#extendcriteria').html("Amount must be >= " + lockAmount + ", locktime must exceed " + monthsLeft + " months!");
                                    calculateWeight($("#slider").val());
// @1.0<-
					  	  		}
				  	  		} else {
				  	  			...
				  	  			if(data['witnessaccountdetails'][value['label']]['status'] == "Not funded") {
					  	  			witnesspanelbody += "<li><a data-toggle='modal' href='#fundwitnessaccount' onclick=\"changeWitnessAccount('"+value['UUID']+"')\"><i class='glyphicon glyphicon-credit-card'></i> Fund holder account</a></li>";
					  	  		}
		  	  		...
		  	  		if(data['witnessaccountdetails'][value['label']]['status'] != "Not funded") {
		  	  			...
		  	  			*/
							...
							witnesspanelbody += "<div class='col-xs-12 col-sm-12 col-md-12 col-lg-12'>"; //Primary info div
								...
								witnesspanelbody += "Amount locked: " + data['witnessaccountdetails'][value['label']]['amount'] + " Novo<br>";
								witnesspanelbody += "Locking time: " + data['witnessaccountdetails'][value['label']]['lock_period_time'] + "<br>";
								witnesspanelbody += "Last holding action: " + data['witnessaccountdetails'][value['label']]['last_active_date'] + "<br>";
								witnesspanelbody += "Novo earned: " + data['witnessaccountdetails'][value['label']]['totalearnings'] + " Novo<br>";
							...
							witnesspanelbody += "<div class='col-lg-12 col-md-12 collapse' id='" + moreinfocollapsename + "moreinfo'>"; //Secondary info div
								...
								witnesspanelbody += "Holding cycles: " + data['witnessaccountdetails'][value['label']]['totalcycles'] + "<br>";
								witnesspanelbody += "Novo available: " + data['witnessaccountdetails'][value['label']]['earningsavailable'] + " Novo<br>";
								witnesspanelbody += "Novo immature: " + data['witnessaccountdetails'][value['label']]['earningsimmature'] + " Novo<br>";
								...
								witnesspanelbody += "Estimated holding period: " + data['witnessaccountdetails'][value['label']]['estimated_witness_period'] + " days<br>";
								witnesspanelbody += "Expected earnings: " + data['witnessaccountdetails'][value['label']]['expectedearnings'] + " Novo (" + data['witnessaccountdetails'][value['label']]['expectedearningspercentage'] + "%)<br>";
		  	  ...
		  	  $('#witnessnetworkpanel').html("Novo-daemon is not running");
		  }
	   });
	};
