	...
	var loadjsondata = function() {
	  $.getJSON( "ajax/dashboard.php", function( data ) {
		  ...
		  if(data['server']['cpu']!='') {
			  ...
			  $('#guldenglyph').attr('title', 'Novo-daemon server up to date')
			  				   .tooltip('fixTitle')
		  } else {
		  	  $('#gulden').html("Novo-daemon is not running");
			  $('#node').html("Novo-daemon is not running");
			  $('#witness').html("Novo-daemon is not running");
			  $('#server').html("Novo-daemon is not running");
			  $('#guldenglyph').attr('title', 'Novo-daemon server is not up to date')
			  				   .tooltip('fixTitle')
		  }
		  ...
		  if(data['witness']['phase']=='') { 
		  		...
		  		$('#witnessglyph').attr('title', 'No holding activity')
			  					  .tooltip('fixTitle')
		  	} else { 
		  		$("#witnessdiv").css("background-color","#e6ffe6");
		  		$('#witnessglyph').attr('title', 'Current holding phase')
			  					  .tooltip('fixTitle')
		  	}
		  if(data['server']['cpu']=='') { 
		  		$("#serverdiv").css("background-color","#ffe6e6");
		  		$('#serverglyph').attr('title', 'Novo-daemon server is not running')
			  					 .tooltip('fixTitle')
		  	} else { 
		  		$("#serverdiv").css("background-color","#e6ffe6"); 
		  		$('#serverglyph').attr('title', 'Novo-daemon server is running fine')
			  					 .tooltip('fixTitle')
		  	}
	   });
	};
